...
std::string MemoryData::_canadaString;
std::string MemoryData::_catalogString;
std::string MemoryData::_twitterString;
...
int main(int argc, char **argv) {
    ...
    MemoryData::_canadaString = std::string(std::istreambuf_iterator<wchar_t>(canadaStream), {});

    std::wifstream catalogStream("data/citm_catalog.json");
    catalogStream.imbue(std::locale(std::locale(), new std::codecvt_utf8<char>));
    MemoryData::_catalogString = std::string(std::istreambuf_iterator<wchar_t>(catalogStream), {});

    std::wifstream twitterStream("data/twitter.json");
    twitterStream.imbue(std::locale(std::locale(), new std::codecvt_utf8<char>));
    MemoryData::_twitterString = std::string(std::istreambuf_iterator<wchar_t>(twitterStream), {});
