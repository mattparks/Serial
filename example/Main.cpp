...
int main(int argc, char **argv) {
    ...
    node["array2"]->set(std::vector{1.0f, 10.0f, -5.55f, 9.3456f});
    ...
    node["users"][0]->set(test::User{"user1", "User One", "C++ developer", false, "10/07/2000"});
    node["users"][1]->set(test::User{"user2", "User Two", "University student", true, "11/05/1999"});
    node["users"][3]->set(test::User{"user3", "User Three", "Website developer", false, "2/03/1996"});
    Node user4;
    user4.set(test::User{"user4", "User Four", "High school student", true, "30/04/2002"});
    *node["users"][6] = std::move(user4);
    //node["users"][7] = test::User{"WDR Sinfonieorchester Köln", "Orchestre Symphonique d'Etat de São Paulo", R"(\"Hello World\")", true, "01/00/2000"};
    ...
    Node numbers;
    numbers["a"]->set(std::vector{1, 2, 3, 4});
    numbers["b"]->set(std::vector{5, 6, 7, 8});
    numbers["c"]->set(std::vector{9, 10, 11, 12});
    //numbers["d"] = numbers["a"];
    *node["numbers"] = std::move(numbers);
    ...
    /*{
        ...
        yaml1.writeStream<Yaml>(outStream1, NodeFormat::Beautified);
    }*/
    /*{
        Node xml1;
